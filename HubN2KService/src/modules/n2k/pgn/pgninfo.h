...
struct PGNInfo {
  ...
  PGNInfo(uint32_t pgn, bool singleFrame, uint32_t proprietaryCode, NetworkDataSource source) {
    PGN = pgn;
  ...
  PGNInfo(uint32_t pgn, bool singleFrame, uint32_t proprietaryCode, const bool loopBack) {
    PGN = pgn;
    SingleFrame = singleFrame;
    ProprietaryCode = proprietaryCode;
